...
const VolumeServices = () => {
  ...
  const formatLegendValue = (name, legendObj) => {
    ...
    const totalValue = state?.reduce((acc, cur)=>{
      if(Object.keys(cur).includes(legendObj.dataKey)){ // name.dataKey가 cur객체의 key에 포함돼 있는지 확인
        return acc + cur[legendObj.dataKey] // 포함돼 있다면 acc에 누적해서 더함
        } else {
          return acc;
        }
    }, initialValue)
    ...
    return (
      <span className='custom-legend-item-text-group flex items-center gap-[5px]'>
        ...
        <span className='custom-legend-item-text text-xs font-semibold dark:text-neutral-300'>
          {totalValue}
        </span>
      </span>
    )
  ...
  return (
    <div className='block-wrap lg:my-[14px] w-full sm:w-[calc(50%-7px)] lg:w-auto sm:ml-[14px]'>
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          <Legend iconType='dot' iconSize={10} formatter={formatLegendValue}/>
          <Bar dataKey="volume" fill="#0095ff" radius={[0,0,4,4]} barSize={16} stackId='a' />
          <Bar dataKey="services" fill="#00e096" radius={[4,4,0,0]} barSize={16} stackId='a' />
        </BarChart>
      </ResponsiveContainer>
      </div>
    </div>
